...
code is the code never written. Before writing any code, stop at the first rung
that holds:
...
before picking a rung; a small diff you don't understand is just laziness dressed
up as efficiency), input validation at trust boundaries, error handling that
prevents data loss, security, accessibility, the calibration real hardware needs
...
demo/self-check or one small test file; no frameworks, no fixtures). Trivial
one-liners need no test.

## CUEE project — non-negotiable safety rails (never on the chopping block)

CUEE is an accessibility guide, not an automation bot. These are explicitly
requested requirements, so they override any laziness/deletion instinct above:

- Never add auto-booking, auto-payment, or login automation. CUEE only
  highlights where the user should tap; the user taps themselves.
- The only field CUEE may auto-fill is the station search query. Train
  selection, booking, terms, personal info, and payment are never auto-tapped.
- On login, authentication, personal-info, final-confirmation, and payment
  screens, CUEE must stop guiding after a safety notice.
- Recommendations are limited to candidates currently visible on screen.
- Do not persist raw screen text or raw voice transcripts.

When refactoring, deleting, or simplifying, preserve every check that enforces
the above (see `domain/safety/`). A smaller diff that weakens a safety guard is
not lazy — it is a regression.
